...
/**
 * Tidy a pasted draft into the house format so an approved entry looks like
 * every other entry: ### headings, bullets, and pipe tables. Wording is kept;
 * only the layout changes.
 */
const RULES = `You format CFS Recovery coaching curriculum text. You are given a draft entry and you return the same entry, reformatted.
...
- Do not add a title at the top; the entry's title is stored separately.
- Return ONLY the formatted text. No preamble, no explanation, no code fences.`;
...
export async function POST(req) {
  ...
  const r = await fetch(`${base}/v1/messages`, {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: "user",
        content: `Entry title: ${title || "(untitled)"}\n\nDraft to reformat:\n\n${String(text).slice(0, 20000)}`,
      }],
    }),
  });
